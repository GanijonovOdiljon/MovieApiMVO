...
import com.example.movieappmvi.databinding.FragmentMovieBinding
...
class MovieFragment : Fragment(R.layout.fragment_movie){
    ...
    private lateinit var vievModel: MovieViewModel
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        _binding = FragmentMovieBinding.bind(view)
        vievModel = ViewModelProvider(this)[MovieViewModel::class.java]
        initViews()
    }
    private fun initViews(){
        ...
        binding.btnGet.setOnClickListener {
            viewLifecycleOwner.lifecycleScope.launch{
                vievModel.userIntent.send(MainIntent.FetchUsers(1))
                binding.btnGet.isVisible = false
            }
        }
        viewLifecycleOwner.lifecycleScope.launch{
            vievModel.state.collect{
                when(it){
